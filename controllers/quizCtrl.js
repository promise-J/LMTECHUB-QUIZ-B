...
export const createQuiz = async (req, res) => {
  ...
  const quizObject = {};
    const quizExists = await Quiz.findOne({ title, userId: req.user._id });
    if (quizExists)
      return res.json({
        success: false,
    ...
    quizObject.userId = req.user._id
...
export const viewQuiz = async (req, res) => {
  try {
    ...
    const isvalid = isValidObjectId(quizId)
    if (quizId && isvalid) {
...
export const viewQuizQuestions = async (req, res) => {
  try {
    ...
    const questions = await Question.find({ quizId, userId: req.user._id }).populate("quizId");
    ...
    if (quizId) {
      ...
      if (quiz) {
        if(existUserResponse){
          await Response.findByIdAndDelete(existUserResponse._id)
          const canIdx = quiz.candidates.findIndex((c) => c == candidate);
          quiz.candidates.splice(canIdx, 1);
          await quiz.save();
